...
//bool WM8960::begin(TwoWire &wirePort)
bool WM8960::begin(void)
{
    void *virt_addr;
    ...
    *(volatile uint32_t*)virt_addr = 0; // reset everything because we are resetting the codec
    time_counter = (uint64_t)ms.count();
    while(((uint64_t)ms.count() - time_counter) < 10)
    {
      ms = duration_cast< milliseconds >(
        system_clock::now().time_since_epoch()
      );
    }
    *(volatile uint32_t*)virt_addr = CONTROL_SW_RESETN;
...
void WM8960::enableI2S()
{
  void *virt_addr;
  virt_addr = (char *)map_base + CONTROL;
  *(volatile uint32_t*)virt_addr = CONTROL_I2S_ENABLE | CONTROL_SW_RESETN;
}
...
bool WM8960::isConnected()
{
    ...
    // todo: get ack back from i2c module
    return true;
}
...
bool WM8960::writeRegister(uint8_t reg, uint16_t value)
{
  ...
  uint32_t tulip_reg = 0;

  milliseconds ms = duration_cast< milliseconds >(
      system_clock::now().time_since_epoch()
  );

  uint64_t time_counter = (uint64_t)ms.count();

  virt_addr = (char *)map_base + I2C_STATUS;
  read_result = *(volatile uint32_t*)virt_addr;
  while((read_result & I2C_STATUS_DIN_READY) == 0)
  {
    read_result = *(volatile uint32_t*)virt_addr;
    if(((uint64_t)ms.count() - time_counter) > 100)
    {
      printf("i2c timeout\n");
      return 0; // failure
    }
  ...
  tulip_reg |= (WM8960_ADDR & I2C_CONTROL_DEVICE_ADDRESS_MASK)    << I2C_CONTROL_DEVICE_ADDRESS_SHIFT;
  tulip_reg |= (reg         & I2C_CONTROL_REGISTER_ADDRESS_MASK)  << I2C_CONTROL_REGISTER_ADDRESS_SHIFT;
  tulip_reg |= (value       & I2C_CONTROL_REGISTER_WR_DATA_MASK)  << I2C_CONTROL_REGISTER_WR_DATA_SHIFT;

  virt_addr = (char *)map_base + I2C_CONTROL;
  *(volatile uint32_t*)virt_addr = tulip_reg;
  ...
  if(!(read_result & I2C_STATUS_ACK_0) || !(read_result & I2C_STATUS_ACK_1) || !(read_result & I2C_STATUS_ACK_2))
  {
    printf("codec did not ack\n");
    return 0; // failure
  }
